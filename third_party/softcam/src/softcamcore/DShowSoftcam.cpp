...
Softcam::Softcam(LPUNKNOWN lpunk, const GUID& clsid, HRESULT *phr) :
    CSource(NAME("OnlyAir Virtual Camera"), lpunk, clsid),
    ...
    m_framerate(m_frame_buffer.framerate())
{
    ...
    // CSource::AddPin().
    (void)new SoftcamStream(phr, this, L"OnlyAir Video Stream");
}
...
SoftcamStream::SoftcamStream(HRESULT *phr,
                         Softcam *pParent,
                         LPCWSTR pPinName) :
    CSourceStream(NAME("OnlyAir Video Stream"), phr, pParent, pPinName),
